...
export function isWithinAge(value: string | null, maxAgeDays: number, now = new Date()): boolean {
...
function parseListingDate(value: string | null, now: Date): Date | null {
    if (!value) return null;

    const normalized = value.toLocaleLowerCase('pl-PL');
    if (normalized.includes('dzisiaj')) return startOfDay(now);
